"""
    ...
    This file includes:
     * Enzyme/non-enzyme classification training code
...
parser.add_argument('--non_num', default=60)
...
class CustomProteinDataset(Dataset):
    """
    For each epoch feature map needs to be generated again with a new set of random, non-catalytic residues.
    """
    ...
    def update_saliency_maps(self, nonposition_num, nonposition_value):
        # Generate feature map (saliency maps, but really, activation map) by reassigning random positions for the training set.
...
def main():
    ...
    set_seed(42)
    csv_file_path = 'datasets/uniprot_sequences_with_positions_training.csv'
    ...
    non_enzyme_file = 'datasets/non_enzyme_new_data_sequence.txt'
    ...
    non_num = parser.parse_args().non_num
    alpha = parser.parse_args().alpha
    
    # Creates saliency maps for training and validation
    
    train_saliency = create_saliency_maps_custom(imp_train_np, SEQ_LENGTH, non_num, 0.01)
    val_saliency = create_saliency_maps_custom(imp_val_np, SEQ_LENGTH, non_num, 0.01)
    ...
    for epoch in range(EPOCHS):
        ...
        new_val_accuracy = val_correct / val_total
        if new_val_accuracy > val_accuracy:
            model_filename = f'high_attention_NONNUM_{non_num}_ALPHA_{alpha}_EPOCH_{epoch}.pth'
